...
// If the sniffer_sa itself needs to publish, uncomment the pubsub_publisher block.============================================================== ATTESA PUB/SUB ADMIN
/*
...
resource "google_logging_metric" "sniffer_heartbeat_metric" {
  ...
  metric_descriptor {
    metric_kind = "DELTA" // OK for INT64 counter
    value_type  = "INT64"
    unit        = "1"
    labels {
      key         = "sniffer_id"
...
// UDM Events Generated Metric: Distribution of UDM events generated per processing job.
resource "google_logging_metric" "udm_events_generated_metric" {
  ...
  description = "Distribution of UDM events generated by the processor per file."

  metric_descriptor {
    metric_kind = "CUMULATIVE" // MODIFIED: Must be CUMULATIVE for DISTRIBUTION in PromQL
...
resource "google_logging_metric" "pcap_download_success_processor" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"INFO\" AND jsonPayload.message=~\"Download complete for\""
  description = "Counts successful PCAP downloads by the processor."
  metric_descriptor { metric_kind = "DELTA" value_type  = "INT64" unit = "1" } // OK for INT64 counter
}
...
resource "google_logging_metric" "pcap_download_notfound_processor" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"ERROR\" AND jsonPayload.message=~\"Error: pcap gs://.* not found.\""
...
resource "google_logging_metric" "tshark_conversion_success_processor" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"INFO\" AND jsonPayload.message=~\"tshark conversion successful:\""
...
resource "google_logging_metric" "tshark_conversion_error_processor" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"ERROR\" AND jsonPayload.message=~\"Subprocess error \\\\(tshark\\\\):\""
...
resource "google_logging_metric" "udm_packet_processing_errors" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"WARNING\" AND jsonPayload.message=~\"UDM_PACKET_ERRORS:\""
  ...
  value_extractor = "REGEXP_EXTRACT(jsonPayload.message, \"UDM_PACKET_ERRORS: ([0-9]+)\")"
  bucket_options {
    linear_buckets {
...
resource "google_logging_metric" "udm_upload_success_processor" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"INFO\" AND jsonPayload.message=~\"Upload complete for .*udm.json\""
...
resource "google_logging_metric" "processor_pcap_latency" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"INFO\" AND jsonPayload.message=~\"PROCESSING_DURATION_SECONDS:\""
  ...
  value_extractor = "REGEXP_EXTRACT(jsonPayload.message, \"PROCESSING_DURATION_SECONDS: ([0-9]+\\\\.?[0-9]*)\")"
}
