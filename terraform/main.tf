...
resource "google_logging_metric" "udm_events_generated_metric" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND textPayload=~\"INFO - Successfully wrote [0-9]+ UDM events to\""
...
resource "google_logging_metric" "udm_packet_processing_errors" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND textPayload=~\"WARNING - UDM_PACKET_ERRORS:\""
