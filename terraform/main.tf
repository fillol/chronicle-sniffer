...
resource "google_logging_metric" "udm_events_generated_metric" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"INFO\" AND textPayload=~\"Successfully wrote [0-9]+ UDM events to\""
  ...
  value_extractor = "REGEXP_EXTRACT(textPayload, \"Successfully wrote ([0-9]+) UDM events to\")"
}
...
resource "google_logging_metric" "pcap_download_success_processor" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"INFO\" AND textPayload=~\"Download complete for\""
...
resource "google_logging_metric" "pcap_download_notfound_processor" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"ERROR\" AND textPayload=~\"Error: pcap gs://.* not found.\""
...
resource "google_logging_metric" "tshark_conversion_success_processor" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"INFO\" AND textPayload=~\"tshark conversion successful:\""
...
resource "google_logging_metric" "tshark_conversion_error_processor" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"ERROR\" AND textPayload=~\"Subprocess error \\\\(tshark\\\\):\""
...
resource "google_logging_metric" "udm_packet_processing_errors" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"WARNING\" AND textPayload=~\"UDM_PACKET_ERRORS:\""
...
resource "google_logging_metric" "udm_upload_success_processor" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"INFO\" AND textPayload=~\"Upload complete for .*udm.json\""
...
resource "google_logging_metric" "processor_pcap_latency" {
  ...
  filter      = "resource.type=\"cloud_run_revision\" AND severity=\"INFO\" AND textPayload=~\"PROCESSING_DURATION_SECONDS:\""
